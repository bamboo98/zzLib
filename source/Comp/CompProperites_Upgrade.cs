...
namespace zzLib.Comp
{
    public class CompProperties_Upgrade : CompProperties
    {
        ...
        // multipliers to stats that will come into effect once the upgrade has been completed
        //这是加的
        public List<StatModifierOffset> statModifiersOffset = new List<StatModifierOffset>(0);
        ...
        // minimum int skill level on the builder who can install the upgrade
        public int intSkillPrerequisite = 0;

        // 允许无限次数升级
        public bool unlimitedUpgrade = false;
        // 允许最大升级次数
        public int limitedUpgrade = 1;
        ...
        public string EffectDescription
        {
            get
            {
                if (_effectDescription == null)
                {
                    ...
                    if (statModifiersOffset.Count > 0)
                    {
                        if (effectDescription != null) s.AppendLine();
                        for (var i = 0; i < statModifiersOffset.Count; i++)
                        {
                            var effect = statModifiersOffset[i];
                            s.Append(effect.stat.LabelCap);
                            s.Append(": ");
                            s.Append(effect.ValueToStringAsOffset);
                            if (i < statModifiersOffset.Count - 1) s.AppendLine();
                        }
                    }
                    if (statModifiers.Count > 0)
                    {
                        ...
                        for (var i = 0; i < statModifiers.Count; i++)
                        {
                            ...
                            if (i < statModifiers.Count - 1) s.AppendLine();
                        }
                    }
        ...
        public string GetPrerequisitesDescription(ThingDef parentDef)
        {
            if (_prerequisitesDescription == null)
            {
                ...
                if (intSkillPrerequisite > 0)
                {
                    reqList.Add("Upgrade_prerequisitesSkill_int".Translate(intSkillPrerequisite));
                }
